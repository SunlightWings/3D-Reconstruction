...
function FinalModelComparison() {
  ...
  return <section className="final-model-preview" aria-label="Final corrected confidence-aware model comparison">
    ...
    <div className="final-model-body"><div className="final-model-images"><ImageCard title="Ground truth" src={image("ground_truth")} subtitle={`${scene.label} · frame ${frame}`} /><ImageCard title="W3A3+ Resolve" src={image("after_corrector")} subtitle="Corrector + confidence-aware output" /></div><div className="final-model-scores" aria-label="Final model scores pending"><div><small>PSNR ↑</small><b>—</b><span>Metrics pending</span></div><div><small>SSIM ↑</small><b>—</b><span>Metrics pending</span></div><div><small>LPIPS ↓</small><b>—</b><span>Metrics pending</span></div></div></div>
  </section>;
}
