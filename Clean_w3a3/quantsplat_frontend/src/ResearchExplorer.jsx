...
function useResearchData() {
  ...
  useEffect(() => {
    Promise.all(["catalog.json", "geometry.json", "runtime.json", "turntable/manifest.json", "focal_calibration_metrics.json"].map((name) =>
      fetch(asset(name)).then((response) => response.ok ? response.json() : name.includes("turntable") ? {} : Promise.reject(new Error(name))),
    )).then(([catalog, geometry, runtimes, turntable, focalMetrics]) => setData({ catalog, geometry, runtimes, turntable, focalMetrics }))
...
function FinalModelPreview({ scene, view }) {
  return <section className="final-model-preview" aria-label="Upcoming final model preview">
    <div className="final-model-header"><div><p className="eyebrow">UPCOMING FINAL MODEL</p><h3>W3A3+ Resolve</h3><p>Corrected and confidence-aware reconstruction</p></div><span>Renders & metrics pending</span></div>
    <div className="final-model-body"><div className="final-model-empty"><b>Final-model render</b><small>{scene.label} · held-out frame {scene.frames[view]}</small><span>Awaiting the corrected confidence-aware output</span></div><div className="final-model-scores" aria-label="Final model scores pending"><div><small>PSNR ↑</small><b>—</b><span>Pending</span></div><div><small>SSIM ↑</small><b>—</b><span>Pending</span></div><div><small>LPIPS ↓</small><b>—</b><span>Pending</span></div></div></div>
  </section>;
}

function Compare({ catalog, focalMetrics }) {
...
export default function ResearchExplorer() {
  ...
  const tabs = [["compare", "Compare views"], ["turntable", "Precomputed 3D"], ["results", "Results"], ["experiments", "Experiments"], ["diagnostics", "Geometry & timing"], ["method", "Method & data"]];
