...
DIAG_HEAVY_ROOT = Path("/var/tmp/luli38se/quantsplat/downstream_diagnostics_v1")

# Training/rendering iteration count for the whole pipeline.
#
# 7000, not 30000. Measured justification, on this dataset:
#   results/downstream_3dgs/convergence_7k_vs_30k.json (40 scenes)
#       7k -> 30k gains Full +0.095 dB, Quant +0.065 dB, and SSIM gets WORSE by -0.016
#   ongoing_logs.md #003 (oracle sweep, 8 scenes)
#       7k 14.023 dB -> 30k 14.061 dB, i.e. +0.038 dB for 4.3x the compute
#
# NOTE: every result published before 2026-09-08 -- DIAGNOSTIC_RESULTS.md,
# aggregate_metrics.json, and ongoing_logs.md entries #001-#024 -- was computed at
# 30000. Numbers produced at 7000 are NOT interchangeable with those; re-running a
# gate at 7000 produces a new number that must be logged as its own entry.
# All arms in any single comparison must use the SAME value, or bit-width/init is
...
def scene_manifest(category: str, sequence: str) -> Dict[str, Any]:
    return read_json(dv.OUT_ROOT / category / sequence / "scene_manifest.json")
