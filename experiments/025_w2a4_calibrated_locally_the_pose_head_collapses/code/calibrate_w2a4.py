...
QUANTVGGT = Path("/home/utn/luli38se/cv/QuantVGGT")
...
DEFAULT_CALIB = Path("/var/tmp/luli38se/quantsplat/models/QuantVGGT/calib_data.pt")
...
def main():
    ...
    # --- ablation switches: each removes one component of QuantVGGT's pipeline ---
    ap.add_argument("--no-smooth", action="store_true",
                    help="ablate SmoothQuant-style per-channel scaling (channel_wise_scale)")
    ap.add_argument("--no-rot", action="store_true",
                    help="ablate the QuaRot Hadamard rotation (rotation_matrix)")
    ap.add_argument("--no-lwc", action="store_true",
                    help="ablate learned weight clipping (clip_factor_w_max/min)")
    ap.add_argument("--no-lac", action="store_true",
                    help="ablate learned activation clipping")
    a = ap.parse_args()
    ...
    config.update_from_args(
        ...
        not_smooth=a.no_smooth, not_rot=a.no_rot,
        lwc=not a.no_lwc, lac=not a.no_lac, rv=True,
        exp_name=a.exp_name,
    )
    ...
    print(f"W-bit={config.w_bits}  A-bit={config.a_bits}")
    print(f"smooth={not config.not_smooth}  rot={not config.not_rot}  "
          f"LWC={config.lwc}  LAC={config.lac}")
    print(f"epochs={config.epochs}  nsamples={config.nsamples}  cali_bsz={config.cali_bsz}")
    print(f"exp_dir={config.exp_dir}")

    # These four switches change the MODULE STRUCTURE (rotation_matrix, channel_wise_scale,
    # clip_factor_w_*). A loader configured differently would silently drop those tensors via
    # load_state_dict(strict=False) and quietly evaluate the wrong model, so the config is
    # written next to the artifacts and quant_loader.py reads it back rather than assuming.
    needs_calib = (not config.not_smooth) or config.lac or config.lwc
    if not needs_calib:
        print("NOTE: smooth/lwc/lac all disabled -> QuantVGGT performs no calibration "
              "(round-to-nearest weights). This runs in seconds, not hours.")
    Path(config.exp_dir).mkdir(parents=True, exist_ok=True)
    (Path(config.exp_dir) / "ablation_config.json").write_text(json.dumps({
        "wbit": a.wbit, "abit": a.abit, "exp_name": a.exp_name,
        "not_smooth": bool(config.not_smooth), "not_rot": bool(config.not_rot),
        "lwc": bool(config.lwc), "lac": bool(config.lac), "rv": True,
        "epochs": config.epochs, "nsamples": config.nsamples,
        "calibration_performed": bool(needs_calib),
        "calib_path": str(a.calib_path),
    ...
    if not produced and needs_calib:
        raise SystemExit("calibration produced no qs_*_parameters_total.pth -- treat as FAILED")
    if not produced:
        # Expected for the round-to-nearest ablation: nothing is learned, so there is
        # nothing to persist. quant_loader.py rebuilds this variant from config alone.
        print("no qs artifacts (round-to-nearest ablation) -- config written, nothing to learn")
