...
CV_PY = Path("/home/utn/luli38se/cv/.venv/bin/python")
GS_PY = Path("/opt/saltstack/salt/bin/python3.10")
GS_SITE = Path("/var/tmp/luli38se/3dgs-site")

PRED_ROOT = Path(
    "/var/tmp/luli38se/quantsplat/disagreement_dataset_v1/run/predictions"
)
CO3D_ROOT = Path("/var/tmp/luli38se/co3d_single_all")
OUT_ROOT = Path("/var/tmp/luli38se/quantsplat/downstream_validation_v1")
...
def evaluate_scene(
    ...
    for i, frame in enumerate(heldout_frames):
        ...
        mask = content_masks[i]
        row = {
            "frame": int(frame),
            "full_psnr_content": psnr(fr, gt, mask),
            "quant_psnr_content": psnr(qr, gt, mask),
            "full_ssim_content": ssim_value(fr, gt, mask),
            "quant_ssim_content": ssim_value(qr, gt, mask),
            "full_lpips_content": lpips_crop(fr, gt, mask),
            "quant_lpips_content": lpips_crop(qr, gt, mask),
        }
        row["full_minus_quant_psnr"] = row["full_psnr_content"] - row["quant_psnr_content"]
        row["full_minus_quant_ssim"] = row["full_ssim_content"] - row["quant_ssim_content"]
        row["quant_minus_full_lpips"] = row["quant_lpips_content"] - row["full_lpips_content"]
        rows.append(row)
    ...
    summary = {
        "n_views": n,
        "primary_region": "unpadded real-image content",
